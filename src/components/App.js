...
export default function App() {
  ...
  const [fromValues, setFromValues] = useState(initialFormValues) // fix this using the state hook

  const updateForm = (inputName, inputValue) => {
    // 🔥 STEP 8 - IMPLEMENT a "form state updater" which will be used inside the inputs' `onChange` handler
    //  It takes in the name of an input and its value, and updates `formValues
    setFromValues({ ...fromValues, [inputName]: inputValue })
  }
  ...
  const submitForm = () => {
    ...
    const newFriend = {
      username: fromValues.username.trim(),
      email: fromValues.email.trim(),
      role: fromValues.role, 
    }
